...
// Supports: Telegram bot, Email (via Resend), local file, or stdout (default).
//
// Usage:
...
//   - "file": saves as a dated markdown file locally (needs outputDir)
...
// Saves the digest as a dated markdown file locally.
// No API keys needed. Files are named like "digest-2026-05-26.md".
async function sendFile(text, outputDir) {
  const dir = outputDir || join(USER_DIR, 'digests');
...
async function main() {
  ...
  // Allow environment variables to override config (for CI/CD)
  const delivery = {
    method: process.env.FB_DELIVERY_METHOD || config.delivery?.method || 'stdout',
    chatId: process.env.FB_DELIVERY_CHAT_ID || config.delivery?.chatId,
    email: process.env.FB_DELIVERY_EMAIL || config.delivery?.email,
    outputDir: process.env.FB_DELIVERY_OUTPUT_DIR || config.delivery?.outputDir,
  };
  ...
  try {
    switch (delivery.method) {
      ...
      case 'file': {
        ...
        await sendFile(digestText, outputDir);
        break;
      }
